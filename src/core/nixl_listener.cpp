...
private:
    ...
    std::unordered_map<std::string, std::unique_ptr<etcd::Watcher>,
                        std::hash<std::string>, strEqual> remoteWatchers;
...
public:
    nixlEtcdClient(const std::string& my_agent_name) {
        std::string etcd_endpoints = std::getenv("NIXL_ETCD_ENDPOINTS");
    ...
    void setupAgentWatcher(const std::string &agent_name) {
        if (remoteWatchers.find(agent_name) != remoteWatchers.end()) {
            return;
        }
        ...
        std::string agent_prefix = makeKey(agent_name, "");
        remoteWatchers[agent_name] = std::make_unique<etcd::Watcher>(*etcd, agent_prefix, process_response);
    }
